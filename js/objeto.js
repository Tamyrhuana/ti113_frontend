...
function mostrarValores() {
    ...
    pessoa.altura = parseFloat
        (document.querySelector('#txtAltura').value);

    pessoa.idade = parseInt
    ...
    let mensagem = `
   <br><br>
     O nome digitado é ${pessoa.nome} </strong> <br>
     A altura informada é ${pessoa.altura.toFixed(2)} </strong> <br>
     A idade informada é ${pessoa.idade} </strong> <br>
     A data de nascimento informada é ${pessoa.dataNascimento} </strong> `;
...
// let idade = parseInt(prompt("Informe Idade:"));
// let dataNascimento = prompt("Informe Data Nascimento (dd/MM/yyyy");
